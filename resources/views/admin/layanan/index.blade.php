        ...
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach($layanans as $layanan)
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-4 py-3.5 font-medium text-gray-900">{{ $layanan->title }}</td>
                                <td class="px-4 py-3.5 text-gray-500 font-mono text-xs">{{ $layanan->slug }}</td>
                                <td class="px-4 py-3.5">
                                    @if($layanan->image)
                                        <img src="{{ asset('storage/'.$layanan->image) }}" class="h-10 w-16 object-cover rounded-lg border border-gray-200">
                                    @else
                                ...
                                <td class="px-4 py-3.5">
                                    <div class="flex items-center justify-end gap-1">
                                        <a href="{{ route('admin.layanan.edit', $layanan) }}" class="p-1.5 rounded-lg hover:bg-gray-100 text-gray-500 hover:text-blue-600 transition" title="Edit">
                                            <i data-lucide="pencil" class="w-4 h-4"></i>
                                        </a>
                                        <form action="{{ route('admin.layanan.destroy', $layanan) }}" method="POST" class="inline" onclick="return confirm('Yakin ingin menghapus?')">
                                            @csrf @method('DELETE')
            ...
            @if($layanans->isEmpty())
                <div class="text-center py-12">
                    <i data-lucide="briefcase" class="w-10 h-10 text-gray-300 mx-auto mb-3"></i>
