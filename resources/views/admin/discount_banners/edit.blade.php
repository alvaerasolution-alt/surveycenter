...
<div class="p-6 max-w-2xl">
    ...
    <form action="{{ isset($discountBanner) ? route('admin.discount-banners.update', $discountBanner) : route('admin.discount-banners.store') }}" 
        ...
        <div>
            ...
            <input type="text" name="title" value="{{ old('title', $discountBanner->title ?? '') }}" class="border border-gray-300 p-2.5 w-full rounded-lg text-sm focus:ring-2 focus:ring-orange-400 focus:border-transparent outline-none">
        </div>

        <div>
            <label class="block text-sm font-semibold text-gray-700 mb-1">Sub Judul</label>
            <input type="text" name="subtitle" value="{{ old('subtitle', $discountBanner->subtitle ?? '') }}" class="border border-gray-300 p-2.5 w-full rounded-lg text-sm focus:ring-2 focus:ring-orange-400 focus:border-transparent outline-none">
        </div>

        <div class="grid grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Teks Tombol</label>
                <input type="text" name="button_text" value="{{ old('button_text', $discountBanner->button_text ?? '') }}" class="border border-gray-300 p-2.5 w-full rounded-lg text-sm focus:ring-2 focus:ring-orange-400 focus:border-transparent outline-none">
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Urutan</label>
                <input type="number" name="order" value="{{ old('order', $discountBanner->order ?? 0) }}" class="border border-gray-300 p-2.5 w-full rounded-lg text-sm focus:ring-2 focus:ring-orange-400 focus:border-transparent outline-none">
            </div>
        </div>

        <div>
            <label class="block text-sm font-semibold text-gray-700 mb-1">Link Tombol</label>
            <input type="text" name="button_link" value="{{ old('button_link', $discountBanner->button_link ?? '') }}" class="border border-gray-300 p-2.5 w-full rounded-lg text-sm focus:ring-2 focus:ring-orange-400 focus:border-transparent outline-none">
        </div>
        ...
        <div>
            ...
            @if(isset($discountBanner) && $discountBanner->image)
                <img src="{{ asset('storage/'.$discountBanner->image) }}" class="w-24 mt-2 rounded-lg">
            @endif
        </div>
